...
def laser_callback(scan):
	...
	pcl_points_set=removeInlinear(pcl_points,p.extract(indices))

	
	p = pcl.PointCloud(np.array(pcl_points_set, dtype=np.float32))
	...
	marker_array.markers.append(get_line((model[1], model[0]), (model[4], model[3]), 1,(0,1,0)))
	pub.publish(marker_array)
