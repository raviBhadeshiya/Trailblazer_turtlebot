...
class MapMaker:
	...
	def process_scan(self, msg):
		...
		for angle in np.arange(msg.angle_min,msg.angle_max+msg.angle_increment,msg.angle_increment):
			d=dist.next()
			...
			if self.to_grid(x,y)!=None and self.to_grid(self.pose[0],self.pose[1]) != None:
				...
				for i in [1]:#to make thick ++list
					self.mapData[to_index(gx-i,gy,self.size_x)]+=pr
					self.mapData[to_index(gx,gy-i,self.size_x)]+=pr
					self.mapData[to_index(gx+i,gy,self.size_x)]+=pr
					self.mapData[to_index(gx,gy+i,self.size_x)]+=pr
					self.mapData[to_index(gx-i,gy-i,self.size_x)]+=pr-0.4
					self.mapData[to_index(gx+i,gy-i,self.size_x)]+=pr-0.4
					self.mapData[to_index(gx+i,gy+i,self.size_x)]+=pr-0.4
					self.mapData[to_index(gx-i,gy+i,self.size_x)]+=pr-0.4
